  ...
  function($scope, $http, $window, appConfig) {
	...
	$http
	  ...
	  .then(
	    ...
	    (err) => $scope.errorMessage = "We could not get the current season of Survivor. Please try again soon."
	  );
	...
	$scope.removePlayer = function(playerName) {
	  ...
	  if (player) {
	    ...
	    $scope.models.contestants = $scope.models.contestants.concat(player.contestants);
